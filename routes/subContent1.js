...
const subContent1 = require('../models/subContent1');
const List1 = require('../models/subContentListing1');
const Counter = require('../models/counterS2');
const Counter1 = require('../models/listingCounter3');
const multer = require('multer');
const upload1=multer()

router.get('/getContent', async (req, resp) => {
  try {
    const messages = await subContent1.find();
    resp.json(messages);
  } catch (error) {
    console.error(err);
...
// router.get('/getSubcontent_3', async (req, resp) => {
//   try {
//     const messages = await subContent1.find().select({_id:0,seq:1,content:1,subTopicName:1,subTopic:1});
//     resp.json(messages);
//   } catch (error) {
//     console.error(err);
//     resp.status(500).send("some error occured")
//   }
// })
router.post('/getSubcontent_3_ByID',upload1.none(), async (req, resp) => {
  try {
    const {topicID}=req.body
    const messages = await subContent1.find({topicID}).select({_id:0,seq:1,id:1,content:1,isClickable:1});
    resp.json(messages);
  } catch (error) {
    console.error(err);
    resp.status(500).send("some error occured")
...
router.post('/addContent',upload1.none(), async (req, resp) => {
  try {
    ...
    if(inputType==="text"){
      const addContent = new subContent1({
...
router.post('/editClickable',async (req,resp)=>{
  try{
    ...
    const { isClickable ,id} = req.body;
        ...
        let note = await subContent1.findOne({id:id});
        console.log(note)
        if (!note) { return resp.status(404).send("not found") }
        note = await subContent1.updateOne({id:id}, { $set: {isClickable} }, { new: true })
        resp.json({ note })
  }
  catch (error) {
    console.error(err);
...
router.post('/updateContent', async (req, resp) => {
  try {
    ...
    const note = await subContent1.find()
    let note1 = await subContent1.findById(startId);


    let note2 = await subContent1.findById(endId);
